...
<template>
    <footer ref="footerRef" class="fixed bottom-0 left-0 right-0  flex items-stretch px-2  max-w-(--max-app-width) m-auto pointer-events-none">
        <div :class="(isOpen) ? 'ml-0' : 'ml-[calc(100%-30px)]'" class="w-full flex items-center gap-2 pb-1.5 pointer-events-auto">
            ...
            <ShinyContainer class="grow overflow-x-hidden rounded-md">
                <div class="overflow-x-auto flex items-center gap-2 py-1 px-2 rounded-md bg-(--foreground)">
                    <p v-for="button in EDITOR_FOOTER_BUTTONS" :key="button.id" @click="() => { button.function(editorView) }" class="bg-white/15 px-4 rounded-sm">
                        {{ button.label }}
                    </p>
                </div>
            </ShinyContainer>
