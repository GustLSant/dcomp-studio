<script setup lang="ts">
    ...
    const props = defineProps<{ exerciseData: ExerciseData }>();
    ...
    <div class="flex flex-col gap-2 rounded-md border border-(--border-02) p-2 accordion" :class="(open) ? 'max-h-[300px] overflow-y-auto' : 'max-h-[42px] overflow-y-hidden'">
        <div @click="toggleOpen" class="flex items-center justify-between gap-2">
            ...
            <div class="flex items-center gap-1">
                <p>{{ props.exerciseData.name }}</p>
                <Icon v-if="props.exerciseData.completed" class="text-(--success-color)" icon="mdi:check-circle-outline" width="18" height="18" />
        ...
        <div class="flex flex-col gap-1">
            ...
            <div class="bg-neutral-950 p-2 rounded-sm shadow-inner font-mono text-green-500">
                <p class="whitespace-pre">{{ props.exerciseData.expectedCodeOutput }}</p>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between gap-2">
            ...
            <ExerciseCompletedBadge v-if="props.exerciseData.completed" class="text-success" />
